...
public class Main {
	...
	private static void executeOption(Scanner in, String opt, Cliente cl, Trotinete trt, Sistema s) {
		switch (opt) {
		case "ADCLIENTE":
			// Adicionar um novo cliente.
			
			processAdCliente(in,cl);
			break;
		...
		case "DADOSTROT":
			...
			processDadosTrot(in,trt);
			break;
		
		case "CLIENTE":
			//Consulta do cliente que está a alugar a trotinete.
			
			processCliente(in,trt);
			break;
		
		case "CARRSALDO":
			// O saldo atual do cliente identificado pelo NIF é adicionado com o valor dado.
			
			processCarrSaldo(in,cl,s);
			break;
			
		case "ALUGAR":
			// Pedido de aluguer da trotinete pelo cliente.
			
			processAlugar(in,cl,trt);
			break;
		
		case "LIBERTAR":
			// Fim de aluguer da trotinete pelo cliente.
			
			processLibertar(in,cl,trt,s);
			break;
